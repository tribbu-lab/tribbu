...
const _initOneSignal = (attempt = 0) => {
  import("onesignal-cordova-plugin").then(m => {
    ...
    try {
      ...
      _OS.initialize("0b8fa04a-a2ba-4c3e-8b40-d2bec954f7ba");
...
// Obtener IDs de usuarios de un curso
const getUserIdsByCurso = async (cursoId) => {
...
const T = {
  primary: "#0F172A",
  accent: "#3B82F6",
  bg: "#F8FAFC",     // Fondo grisáceo muy tenue para dar profundidad
  card: "#FFFFFF",
  text: "#1E293B",   // Texto principal (más suave que el negro puro)
  muted: "#64748B",  // Texto secundario para fechas y detalles
  border: "#F1F5F9", // Bordes casi invisibles
  white: "#FFFFFF",
  red: "#EF4444",
  green: "#10B981",
  yellow: "#F59E0B",
...
const Card = ({ children, style = {} }) => (
  <div style={{
    ...
    padding: "20px",
    marginBottom: "16px",
    // Sombra suave que genera el efecto de "capas"
...
const Pill = ({ label, color, bg }) => (
  <span style={{
    ...
    fontWeight: 700,
    padding: "4px 12px",
    borderRadius: 100, // Estilo cápsula
    background: bg || "rgba(59, 130, 246, 0.08)", // Fondo muy sutil
    ...
    try {
      ...
      for(const u of inserts) {
        ...
        if(existing?.id) {
          ...
          // Nuevo usuario — crear en Supabase Auth primero
          let auth_id = null;
          try {
            ...
            auth_id = authData?.id || null;
          } catch(authErr) {
            console.error("Error creando en Auth:", u.email, authErr);
          }
          // Hashear contraseña para guardar en nuestra tabla
          ...
          const { error: e } = await supabase.from("usuarios").insert({
            ...
            pass: passHash, avatar: u.avatar, dni: u.dni||null,
...
function SuperAdmin() {
  ...
  const [authSyncMsg,setAuthSyncMsg] = useState(null); // feedback reset Auth
  ...
  const guardarUsuario = async () => {
    ...
    // rol global se deriva de esSuper + cursosAdmin
    ...
    if(modal==="nuevo_usuario") {
      ...
      try {
        ...
        const authRes = await fetch(`${SUPABASE_URL}/auth/v1/admin/users`, {
          ...
          body: JSON.stringify({ email: sanitize(form.email).toLowerCase(), password: form.pass, email_confirm: true }),
        });
        const authData = await authRes.json();
        auth_id = authData?.id || null;
      } catch(e) { console.error("Error creando en Auth:", e); }
      const { data } = await supabase.from("usuarios").insert({nombre:sanitize(form.nombre),apellido:sanitize(form.apellido)||null,email:sanitize(form.email).toLowerCase(),pass:passHash,rol:rolGlobal,avatar,activo:form.activo,dni:sanitize(form.dni)||null,telefono:sanitize(form.telefono)||null,auth_id}).select().single();
      if(data) {
      ...
      const passEditar = passEsNueva ? await bcrypt.hash(passNueva, 10) : form.pass;
      const emailNuevo = sanitize(form.email).toLowerCase();
      await supabase.from("usuarios").update({nombre:sanitize(form.nombre),apellido:sanitize(form.apellido)||null,email:emailNuevo,pass:passEditar,rol:rolGlobal,activo:form.activo,dni:sanitize(form.dni)||null,telefono:sanitize(form.telefono)||null}).eq("id",form.id);
      // Sincronizar email y/o clave en Supabase Auth
      const emailCambio = emailNuevo !== (form._emailOriginal||"").toLowerCase();
      if(passEsNueva || emailCambio) {
        try {
          const serviceKey = import.meta.env.VITE_SUPABASE_SERVICE_KEY;
          if(!serviceKey) { setAuthSyncMsg({ ok:false, msg:"⚠️ VITE_SUPABASE_SERVICE_KEY no configurada. Los cambios se guardaron en la tabla pero no en Supabase Auth." }); }
          else {
            let authId = form.auth_id;
            if(!authId) {
              const res = await fetch(`${SUPABASE_URL}/auth/v1/admin/users?email=${encodeURIComponent(form._emailOriginal||form.email)}`, {
                headers: { "Authorization": `Bearer ${serviceKey}`, "apikey": serviceKey }
              });
              const json = await res.json();
              authId = json?.users?.[0]?.id || null;
              if(authId) await supabase.from("usuarios").update({ auth_id: authId }).eq("id", form.id);
            }
            if(!authId) {
              setAuthSyncMsg({ ok:false, msg:`⚠️ No se encontró el usuario en Supabase Auth. Actualizalo manualmente desde el Dashboard.` });
            } else {
              const body = {};
              if(passEsNueva) body.password = passNueva;
              if(emailCambio) { body.email = emailNuevo; body.email_confirm = true; }
              ...
              if(r.ok) setAuthSyncMsg({ ok:true, msg:`✅ ${[emailCambio?"Email":"",passEsNueva?"Clave":""].filter(Boolean).join(" y ")} actualizado correctamente en Auth para ${emailNuevo}.` });
              else setAuthSyncMsg({ ok:false, msg:`⚠️ Error al actualizar en Auth: ${rj?.message||r.status}.` });
            }
          }
        } catch(e) { setAuthSyncMsg({ ok:false, msg:`⚠️ Error de red al actualizar Auth: ${e.message}` }); }
      }
  ...
  const guardarAlumno = async () => {
    ...
    if(modal==="nuevo_alumno") {
      await supabase.from("hijos").insert({nombre:form.nombre,apellido:form.apellido||null,curso_id:form.curso_id,avatar,color,fecha_nacimiento:form.fecha_nacimiento||null,dni:sanitize(form.dni)||null});
    } else {
      await supabase.from("hijos").update({nombre:form.nombre,apellido:form.apellido||null,curso_id:form.curso_id,fecha_nacimiento:form.fecha_nacimiento||null,dni:sanitize(form.dni)||null}).eq("id",form.id);
    }
    setModal(null); cargar();
  };
  ...
  return (
    <div>
      {/* Feedback banner para reset de Auth */}
      {authSyncMsg&&(
      ...
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {[{id:"usuarios",l:"👤 Usuarios"},{id:"cursos",l:"🏫 Cursos"},{id:"maestros",l:"👨‍🏫 Maestros"},{id:"alumnos",l:"🎒 Alumnos"},{id:"horarios",l:"🕐 Horarios"},{id:"uniformes",l:"👕 Uniformes"},{id:"colegio",l:"🏫 Colegio"},{id:"alertas",l:"🚨 Alertas"},{id:"menu",l:"🍽️ Menú"}].map(t=>(
...
function Muro({ cursoId, cursoNombre, isAdmin, userName, userId, misHijos=[], items=[], cursoIdx=0, setCursoIdx, hijoColorsMap={}, cambiarColorHijo, colorPickerIdx, setColorPickerIdx, onNavigate }) {
...
function FestejoModal({ alumnoId, alumnoNombre, cursoId, userId, festejoExistente, onClose, onSave }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",flexDirection:"column",justifyContent:isMobile?"flex-end":"center",alignItems:isMobile?"stretch":"center",paddingLeft:isMobile?0:220,padding:isMobile?0:20}}>
      <Card style={{padding:0,width:"100%",maxWidth:isMobile?"100%":480,borderRadius:isMobile?"20px 20px 0 0":20,maxHeight:"92vh",display:"flex",flexDirection:"column",margin:0}}>
        {/* Scrollable content */}
        ...
        <div style={{padding:"12px 24px 24px",borderTop:"1px solid #F1F5F9",background:"white",borderRadius:"0 0 20px 20px",flexShrink:0}}>
          <div style={{display:"flex",gap:10}}>
...
function RecordatoriosTab({ cursoId, userId, isAdmin, isSuper=false, active, onBadgeChange }) {
  const [recordatorios, setRecordatorios] = useState([]);
  const [leidosSet,     setLeidosSet]     = useState(new Set());
  const [modal,         setModal]         = useState(null);
...
function App() {
  ...
  // Todos los items son hijos — el cursoId siempre viene de curso_id
