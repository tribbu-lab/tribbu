    ...
    <>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:6}}>
        {items.map((a, i) => a.tipo === "imagen" ? (
          <SignedImg key={i} src={a.url} bucket="adjuntos" alt={a.nombre || "adjunto"} onClick={() => setPreview(a)}
            style={{width:56,height:56,objectFit:"cover",borderRadius:10,border:"1px solid #E2E8F0",cursor:"pointer",display:"block"}}/>
        ) : (
          <button key={i} onClick={() => abrirPdf(a)}
      ...
      {preview && (
        <div onClick={() => setPreview(null)}
          ...
          <SignedImg src={preview.url} bucket="adjuntos" alt={preview.nombre || "adjunto"} style={{maxWidth:"100%",maxHeight:"90vh",borderRadius:12,objectFit:"contain"}}/>
        </div>
      )}
    </>
  );
