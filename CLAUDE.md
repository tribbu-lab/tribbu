...
- `src/components/` — `Card`, `Pill`, `Spinner`, etc., re-exported via `components/index.js` barrel. `Adjuntos.jsx` (`AdjuntosInput`/`AdjuntosList`, `MAX_ADJUNTOS = 3`) is the attachments pattern for recordatorios/eventos: images + PDFs (10 MB cap, client-validated), uploaded to the public `adjuntos` Storage bucket at `{cursoId}/{ts}.{ext}` and persisted as a `jsonb` array of `{url, tipo: "imagen"|"pdf", nombre}` on the row. Mobile mirror: `mobile/components/Adjuntos.jsx`.
...
`usuarios`, `cursos`, `hijos`, `usuario_hijos` (parent↔child), `usuario_cursos` (user↔course with `rol`), `cumples`, `eventos` (+`evento_asistencia`), `recordatorios` (+`recordatorio_leidos`), `colectas` (+`colecta_pagos`), `menu`, `alertas`, `contactos`, plus útiles/libros/uniformes tables. `push_tokens` (Expo push tokens per device, used by the mobile app). `recordatorios.adjuntos` and `eventos.adjuntos` are `jsonb DEFAULT '[]'` arrays of `{url, tipo, nombre}` pointing at the public `adjuntos` Storage bucket (see `specs/adjuntos-en-recordatorios-y-eventos.md`). The full list is in `backup_tribbu.cjs`'s `TABLES` (note: the script backs up rows only, not Storage objects).
